...
private:
    ...
    ros::Publisher cmd_pub_, prearm_pos_pub_, exec_state_pub_, points_tracking_start_pub_;
    ...
    void publishExecState()
    {
        std_msgs::Int32 msg;
        msg.data = static_cast<int>(exec_traj_state_);
        exec_state_pub_.publish(msg);
    }

    void publishPointsTrackingStart(const ros::Time &stamp)
    {
        std_msgs::Float64 msg;
        msg.data = stamp.toSec();
        points_tracking_start_pub_.publish(msg);
    }
    ...
    void execFSMCallback(const ros::TimerEvent &e){
        ...
        Exec_Traj_State_t prev_exec_traj_state = exec_traj_state_;
        ...
        case HOVER:
        {
            ...
            else if (param_.use_ref_txt && is_command_mode_ && state_.mode == mavros_msgs::State::MODE_PX4_OFFBOARD)
            {
                ...
                publishPointsTrackingStart(now_time);
        ...
        if (exec_traj_state_ != prev_exec_traj_state)
        {
            publishExecState();
        }
...
public:
    ...
    void init(ros::NodeHandle &nh){
        ...
        exec_state_pub_ = nh.advertise<std_msgs::Int32>("/ommpc_controller/exec_traj_state", 1, true);
        points_tracking_start_pub_ = nh.advertise<std_msgs::Float64>("/ommpc_controller/points_tracking_start_time", 1, true);
